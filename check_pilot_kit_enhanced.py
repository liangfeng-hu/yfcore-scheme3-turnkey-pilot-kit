...
def run_cmd(cmd: str, cwd: Path, timeout: int = 120) -> bool:
    try:
...
def main() -> int:
    ...
    if s1 != 200:
        cprint(f"❌ Case 1 expected 200, got {s1}", "red")
    ...
    if s2 != 403:
        cprint(f"❌ Case 2 expected 403, got {s2}", "red")
        run_cmd("docker compose logs --tail=200", root, timeout=60)
        run_cmd("docker compose down -v", root, timeout=120)
        return 1
    ...
    if s3 != 403:
        cprint(f"❌ Case 3 expected 403, got {s3}", "red")
        run_cmd("docker compose logs --tail=200", root, timeout=60)
        run_cmd("docker compose down -v", root, timeout=120)
        return 1
